...
class PostRepository(private val context: Context) {
    ...
    fun sendFriendRequest(
        ...
        //  receiver name so they see it in their inbox
        requestDatabase.child(targetUsername).child(myUsername).setValue(requestData)
    ...
    fun getFriendshipStatus(myUsername: String, targetUsername: String): Flow<String> =
        callbackFlow {
            val friendRef = friendsDatabase.child(myUsername).child(targetUsername)
            val requestRef = requestDatabase.child(targetUsername).child(myUsername)
            val listener = object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
...
//                            check request
            ...
            // any change in relationships update the UI
            friendsDatabase.addValueEventListener(listener)
            requestDatabase.addValueEventListener(listener)

            awaitClose {
                friendsDatabase.removeEventListener(listener)
                requestDatabase.removeEventListener(listener)
            }
        }
    ...
    fun toggleLike(postId: String, userId: String) {
        ...
        likeRef.get().addOnSuccessListener { snapshot ->
            if (snapshot.exists()) {
                ...
                //  hasnnot liked it so add their ID
...
//            this dispatch handles the upload, if not written image never uploads
            ...
            fun getFriendRequests(myUsername: String): Flow<List<com.example.fit_buddy.model.FriendRequest>> =
                callbackFlow {
            ...
            fun getPostsByUser(userId: String): Flow<List<FeedPost>> = callbackFlow {
                ...
                val listener = object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        val items =
                            snapshot.children.mapNotNull { it.getValue(FeedPost::class.java) }
